...
def _row(**overrides: str) -> dict[str, str]:
    row = {
        ...
        "K_parent": "4",
        "K_child": "4",
        "child_packet_id": "K4_cell1",
        "parent_packet_id": "K4_cell0",
        "child_state": "plus",
        "parent_state": "minus",
        "child_mass": "100",
        "parent_mass": "100",
...
def test_sprint56_packet_local_action_contract_and_fail_closed_route(tmp_path: Path) -> None:
    source = tmp_path / "sprint49_N32_seed0"
    _write_input(
        source,
        [
            _row(time="0.1", child_packet_id="K4_cell1", parent_packet_id="K4_cell0"),
            _row(time="0.2", child_packet_id="K4_cell2", parent_packet_id="K4_cell1", child_state="minus"),
        ],
    )
    out = tmp_path / "out"
    subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "ns_sprint56_packet_local_stretch_action_audit.py"),
            "--inputs",
            str(source),
            "--out-dir",
            str(out),
    ...
    summary = json.loads((out / "ns_sprint56_packet_local_action_summary.json").read_text(encoding="utf-8"))
    action_rows = list(csv.DictReader((out / "ns_packet_local_lagrangian_action.csv").open(newline="", encoding="utf-8")))
    direction_rows = list(csv.DictReader((out / "ns_direction_change_separation.csv").open(newline="", encoding="utf-8")))

    assert summary["contract"] == "ns_sprint56_packet_local_action_artifact"
    assert summary["route_decision"] == "PACKET_LOCAL_MASK_JOIN_INSUFFICIENT"
    assert summary["packet_local_available_lineage_count"] == 0
    assert summary["packet_local_mask_reconstruction_available"] is False
    ...
    assert action_rows
    assert action_rows[0]["packet_local_mask_available"] == "false"
    assert action_rows[0]["direct_stretch_cycle_amplitude"] == "unavailable"
